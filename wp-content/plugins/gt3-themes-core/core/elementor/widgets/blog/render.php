...
if ($settings['query'] instanceof \WP_Query) {
...
if ((bool)$settings['packery_en']) {
	...
	$dataSettings = array(
		'packery' => false,
		'isotope_settings' => array(
			'layoutMode' => $settings['grid_type'] === 'grid' ? 'fitRows' : 'masonry',
		)
	);
}
	...
	<div <?php $widget->print_render_attribute_string('wrapper') ?>>
		...
		if($query->have_posts()) {
			...
			while($query->have_posts()) {
				...
				if($settings['blog_post_listing_content_module'] == 'yes' || (bool)$settings['packery_en'] || gt3_customizer_enabled()) {
